...
def Generate_Data(T,cat_i,n,p,psx,rd_seed):
    ...
    fname='path_to_data/hypergraph_'+str(T)+'_P'+str(psx)+'/covariance_matrix/cat'+str(cat_i)+'_clust_covariance_'
...
def Calculate_Bic(Theta,data,n):
    ...
    for i in range(t):
        ...
        bic+=n[i]*np.trace(theta_psd@COV) - n[i]*np.log(np.linalg.det(theta_psd)) + np.log(n[i])*df + 4*df*np.log(p)
    return(bic)

def errorMetrics(Theta,True_Precission):
    Theta_2 = Theta.copy()
    True_Precission_2 = True_Precission.copy()

    # Mask diagonal
    for k in range(Theta_2.shape[2]):
        np.fill_diagonal(Theta_2[:,:,k], 0)
        np.fill_diagonal(True_Precission_2[:,:,k], 0)
    
    TP=(1*(Theta_2!=0))*(1*(True_Precission_2!=0))
    FP=(1*(Theta_2!=0))*(1*(True_Precission_2==0))
    FN=(1*(Theta_2==0))*(1*(True_Precission_2!=0))
