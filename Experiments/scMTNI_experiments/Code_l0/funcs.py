...
def Calculate_Bic(Theta,data,n,gamma_BIC=1):
    ...
    for i in range(t):
        ...
        COV=np.cov(data[i][:,:p].T)
        
        bic+=n[i]*np.trace(COV@theta_psd) - n[i]*np.log(np.linalg.det(theta_psd)) + np.log(n[i])*df + gamma_BIC*4*df*np.log(p)
    return(bic)
