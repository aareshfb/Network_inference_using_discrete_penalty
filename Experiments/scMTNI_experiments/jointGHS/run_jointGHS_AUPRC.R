...
file_name <- paste0("jointGHS_",N,".csv")
...
if (!file.exists(file_name)) {
  writeLines("T,p,N,Tau1,Lamb2,Recall_1,Precision_1,F1 score-1,Recall_2,Precision_2,F1 score-2,Recall_3,Precision_3,F1 score-3,Recall_avg,Precision_avg,F1 score-avg, BIC,runtime(s), total edges(directed)", file_name)
}
cat("Reading data...\n")
...
Tau1 <- 10^seq(3, -3, length.out = 50)
Lamb2 <- c(0.001) # Not applicable

results <- matrix(0, length(Tau1)*length(Lamb2), 11+3*L)
results[, 1] <- L
results[, 2] <- p
results[, 3] <- N
...
for (i in 1:length(Tau1)){
  for (j in 1:length(Lamb2)){
    ...
    result <- jointGHS(X=data, tau_sq = lamb1*c(1,1,1),AIC_selection = FALSE) # 
    end_time <- proc.time()
    
    masked <- mask_diag(True_Precission, result$theta) #cov2cor passed inside the funciton
