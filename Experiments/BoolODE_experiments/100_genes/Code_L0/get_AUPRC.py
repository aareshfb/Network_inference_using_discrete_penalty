...
nu_values=[0.005,0.001,0.0005,0.0001,0.00005,0.00001]
summary = []

for n in n_values:
    for nu in nu_values:
        file = os.path.join(data_dir, f"{method}_n{n}_g{gamma}_nu{nu}.csv")
        ...
        for j in ["avg"]:
            ...
            summary.append([method, n, gamma, nu, j, round(auprc,2)])
    
summary_df = pd.DataFrame(summary, columns=["Method","n","gamma","nu","Dataset","AUPRC"])
summary_df.to_csv(os.path.join(data_dir, f"{method}_auprc_summary_g{gamma}_all_nu.csv"), index=False)
