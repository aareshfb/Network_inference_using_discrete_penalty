load_dataset <- function(transpose = TRUE) {

  base_path <- "../Expression_data"
  set.seed(0)

  expected_regs <- paste0("reg", 1:15)
  expected_genes <- paste0("gene", 1:50)
  ...
  natural_key <- function(x) {
    ...
    sapply(parts, function(p) {
      if (length(p) == 3) {
        return(sprintf("%s%03d", p[2], as.integer(p[3])))
      } else {
        return(x)
      }
    })
...
read_prec <- function() {
  ...
  for (i in 1:8) {
    ...
    # Check shape
    if (!all(dim(arr) == c(15, 65))) {
      stop(sprintf(
        "%s: expected shape (15, 65), found (%d, %d)",
