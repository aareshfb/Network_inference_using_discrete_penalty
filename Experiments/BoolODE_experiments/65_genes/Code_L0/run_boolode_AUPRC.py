...
def read_prec():
    ...
    expected_regs = [f"reg{i}" for i in range(1, 16)]
    expected_genes = [f"gene{i}" for i in range(1, 51)]
    ...
    for i in range(1, 9):
        ...
        if arr.shape != (15, 65):
            raise ValueError(
                f"{path}: expected shape (15, 65), found {arr.shape}"
            )
...
if __name__ == "__main__":
    ...
    # n=200
    # nu0_list=[1e-2]
    mu_list=np.logspace(2, -6, 200)
    gamma=20

    folder = "masked_diag" if mask_diag else "unmasked_diag"
    path = os.path.join("Output", "AUPRC2", folder)
    ...
    W = np.eye(8)
    edges = [(0, 6), (1, 5), (2, 6), (3, 4), (3, 5), (3, 6), (3, 7)]
    for i, j in edges:
        W[i, j] = 1
    W=np.triu(W,1)
