...
if (!file.exists(file_name)) {
  writeLines("T,p,N,Tau1,Lamb2,Recall,Precision,F1 score, BIC,runtime(s), total edges(directed)", file_name)
}
cat("Reading data...\n")
...
results <- matrix(0, length(Tau1)*length(Lamb2), 11)
results[, 1] <- L
results[, 2] <- p
results[, 3] <- N
...
for (i in 1:length(Tau1)){
  for (j in 1:length(Lamb2)){
    ...
    result <- jointGHS(X=data, tau_sq = lamb1 *rep(1, L), AIC_selection = FALSE) # 
    end_time <- proc.time()
    ...
    for (l in 1:L) {
      for (i in reg_id) {
        result_copy[[l]] <- cov2cor(result_copy[[l]]) #for jointGHS
        result_copy[[l]][i, i] <- 0
